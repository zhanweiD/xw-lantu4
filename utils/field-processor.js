  ...
  try {
    ...
    return `return function (inputData, useProcessor = ${useProcessor}, defaultValue = '${value}', hasSaveCode = ${hasSaveCode}) {
      if (useProcessor) {
        ...
        try {
          return JSON.parse(${typeof defaultValue === 'number' ? '+defaultValue' : 'defaultValue'})
        } catch (error) {
          return defaultValue
        }
      }
...
const getDefaultValue = (value, type) => {
  ...
  const sliceCode = value.slice(value.indexOf('defaultValue ='), value.indexOf(', hasSaveCode'))
...
const getUseProcessor = (value) => {
  ...
  const sliceCode = value.slice(value.indexOf('useProcessor ='), value.indexOf(', defaultValue'))
...
const getHasSaveCode = (value) => {
  ...
  const sliceCode = value.slice(value.indexOf('hasSaveCode ='), value.indexOf(') {'))
  return Boolean(JSON.parse(sliceCode.split(' ')[2]))
}
