...
export function drawAxisX(axisXOption) {
  ...
  const {scale, xTickLineOpacity, xTickSize, xTickColor, xTickAngle, xTickOffset, xTickDirection} = option
  ...
  const axisXContainer = (this.isDev ? this.root.append('g') : this.container.select('svg').append('g'))
    ...
    .attr(
      'transform',
      `translate(${this.isDev ? 0 : this.translateX}, ${
        this.mainHeight + xTickOffset + (this.isDev ? 0 : this.translateY)
      })`
    )
  ...
  xKeys
    ...
    .attr(
      'transform',
      () =>
        `translate(0,${textY + (xTickDirection === STATIC.DIRECTION[1].key ? 0 : xTickSize / 2)})rotate(${xTickAngle})`
    )
