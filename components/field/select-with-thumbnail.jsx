    ...
    <SelectField
      ...
      options={
        props.options.length % 3 !== 0
          ? props.options.concat(Array.from({length: 3 - (props.options.length % 3)}).map(() => ({})))
          : props.options
      }
  ...
  return (
    <components.Option {...props} isDisabled={!data.label}>
      <Grid.Item key={data.key}>
        <div className={c('pa', s.icon, s.position)} style={{backgroundImage: `url(${data.thumbnail})`}} />
  ...
  return (
    <components.SingleValue {...props}>
      <div className="w40 h24 cfw10 fbh fbac fbjc">
        <div className={c('w32', s.iconHeight, s.icon)} style={{backgroundImage: `url(${data.thumbnail})`}} />
      </div>
