...
export const OffsetField = observer(
  ({label, tip, className, value, defaultValue = {}, onChange = () => {}, readOnly}) => {
    const offsetRef = useRef()
    const {margin = [0, 0, 0, 0], padding = [0, 0, 0, 0]} = isDef(value) ? value : defaultValue
    ...
    return (
      <Field label={label} tip={tip} className={className}>
        <div className={c('fbh fbac fbjc pr w100p fieldOffset', s.marginBox)} ref={offsetRef} />
        <div className={c('pa', s.paddingBox)} />
        <div className={c('cfw10 pa', s.contentBox)} />
        {Object.entries(offset).map(([key, itemValue], index) => {
          return (
            <div className={c('pa', s[`${key}Input`])} key={key} style={{width: boxOffset[key] || 20}}>
              <NumberInput
