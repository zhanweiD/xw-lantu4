...
export const MDataSourceModelManager = types
  ...
  .actions((self) => {
    ...
    const beforeOpenUpdateModel = (dataSource) => {
      const {dataSourceId, dataSourceName, dataType, config, remark} =
        dataSource
...
export const MDataSource = types
  ...
  .actions((self) => {
    // 新建数据库源
    const createDataSource = flow(function* createDataSource({
      dataSourceOptions,
      afterCreate = () => {}
    }) {
      const {tip} = self.env_
      ...
      if (dataType === "api" || dataType === "excel" || dataType === "json") {
        ...
        const {dataSourceName, database, host, password, port, type, userName} =
          dataSourceOptions
        try {
          yield self.dataIo_
            .createDataSource({
    ...
    const updateDataSource = flow(function* updateDataSource({
      dataSourceOptions,
      afterUpdate = () => {}
    }) {
      const {tip} = self.env_
