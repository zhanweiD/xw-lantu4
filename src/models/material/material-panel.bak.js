 ...
 * @description 素材管理面板
 */
import {types, getEnv, flow, getParent, applySnapshot, getRoot} from "mobx-state-tree"
import commonAction from "@utils/common-action"
import {createConfigModelClass} from "@components/field"
import {MMaterialThumbnail} from "./material-thumbnail.bak"
...
const MMaterialType = types
  ...
  .views((self) => ({
    get materialList_() {
      return getParent(self, 2).materials.filter((material) => self.type === material.type)
    },
    get env_() {
      return getEnv(self)
    }
...
const MMaterialFolder = types
  ...
  .views((self) => ({
    ...
    get materials_() {
      return self.materials.length && self.materialSort.length ? self.materialSort.map(self.getMaterialById) : self.materials
    }
  }))
  .actions((self) => {
  ...
  .actions((self) => {
    ...
    const toggleFolderTop = flow(function* toggleFolderTop(folder, isTop) {
      if (isTop || self.topFolders.includes(folder.folderId)) {
        self.topFolders = self.topFolders.filter((sortId) => sortId !== folder.folderId)
      } else {
