...
const SectionFields = observer(({sections = [], sessionId, model = {}, className, onChange = () => {}, onListenChangeOption = () => {}, onAction = () => {}}) => {
    ...
    if (sectionFields.length) {
      return (
        <Section
          ...
          sectionConfigField={sectionConfigField && <ModelToField model={sectionConfigField[1]} onAction={onAction} />}
          name={t(sectionName)}
          hideNameBar={sectionName === "__hide__"}
        >
          {sectionFields.map(([key, fieldModel]) => fieldModel.type !== "sectionConfig" && <ModelToField key={key} model={fieldModel} onAction={onAction} />)}
        </Section>
      )
    }
    return null
