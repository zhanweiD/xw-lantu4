...
const SectionFields = observer(
  ({
    ...
    const sectionNames = (
      isStateTreeNode(model.sections) ? model.sections.toJSON() : sections
    ).map((section) => {
    ...
    return sectionNames.map((sectionName) => {
      const sectionFields = fields.filter(([, fieldModel]) => {
        return (
          fieldModel.section === sectionName &&
          fieldModel.whenIsSatisfied === true
        )
      })
      // 目前只取了section.fields中的首个sectionConfig，后续优化支持多个配置
      const sectionConfigField = sectionFields.find(
        ([, fieldModel]) => fieldModel.type === "sectionConfig"
      )
