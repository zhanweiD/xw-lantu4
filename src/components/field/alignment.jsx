...
export const AlignmentField = observer(
  ({label, tip, value, defaultValue, onChange, className, readOnly}) => {
      ...
      <Field label={label} tip={tip} className={className}>
        {OPTIONS.map((option, index) => (
          ...
          >
            ...
            >
              <Icon
                name={`align-${option.value}`}
                size={12}
                fill={readOnly ? "rgba(255,255,255,0.5)" : "#ffffff"}
              />
            </div>
          </Caption>
        ))}
