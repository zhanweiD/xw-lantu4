  ...
  ({
    ...
    const extra =
      isMulti && isArray(value)
        ? value.filter(
            (item) => options.map((option) => option.value).indexOf(item) === -1
          )
        : undefined
      ...
      return (
        <components.Option {...props}>
          <div className={c("fbv fbjc w100p", {h24: isMulti, lh24: isMulti})}>
            <div className={c("omit", {oh: isMulti, [s.label]: isMulti})}>
              {children}
            </div>
...
export const valueTransform = (options = [], selectValue, isMulti, type) => {
  ...
  if (isMulti) {
    ...
    return selectFilter.sort(
      (prev, next) =>
        selectValue.indexOf(prev.value) - selectValue.indexOf(next.value)
    )
  }
  ...
  return {
    option: (provided, state) => {
      ...
      return {
        ...
        borderBottom:
          state.data.label !== state.options.slice(-1)[0].label
            ? "1px solid #1B1B1B"
            : "none",
    ...
    menuList: (provided) => ({
      ...
      backgroundColor:
        type === "select-thumbnail" ? "rgb(0, 0, 0)" : "transparent",
      padding: 0
    }),
    ...
    multiValue: (provided) => ({
      ...provided,
      backgroundColor: readOnlyHighLight
        ? "rgb(0, 126, 255)"
        : readOnly
        ? "rgba(85, 85, 85)"
        : "rgb(0, 126, 255)",
