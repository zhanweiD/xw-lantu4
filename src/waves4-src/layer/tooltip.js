...
export default class Tooltip {
  constructor(container, options) {
    ...
    this.log = createLog("@waves4-src/layer")
  ...
  update({target}, {data, backup}, options = {}) {
    ...
    if (mode === modeType.SINGLE) {
      list = [data].map(({fill, stroke, source}) => ({
        pointColor: fill || stroke,
        ...source
      }))
      ...
      try {
        ...
        const groupData = backup[elType].filter(({source}) =>
  ...
  move({offsetX, offsetY}, options = {}) {
    const {enableAnimation, animationDuration, animationDelay} = {
      ...defaultOptions,
      ...options
    }
    const drift = 10
    ...
    const animation = new MoveAnimation({
      ...
      position: [
        [this.lastPosition.offsetX || 0, offsetX],
        [this.lastPosition.offsetY || 0, offsetY]
      ],
      easing: "easeOutQuart"
    })
    // 一次性动画，结束时销毁
