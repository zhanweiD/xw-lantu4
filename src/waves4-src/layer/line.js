...
export default class LineLayer extends LayerBase {
  ...
  setStyle(style) {
    ...
    const colors = this.getColor(
      this.#curveData[0].length,
      this.#style.curve?.stroke,
      true
    )
    this.#curveData.forEach((groupData) =>
      groupData.forEach((item, i) => (item.color = colors[i]))
    )
    // 标签文字数据
    this.#textData = this.#curveData.map((groupData) =>
      groupData.map(({value, x, y}) => {
    ...
    // 圆点数据
    this.#circleData = this.#curveData.map((groupData) =>
      groupData.map((item) => ({...item, r: circleSize / 2}))
    )
    // 面积数据
    this.#areaData = this.#curveData.map((groupData, i) =>
      groupData.map(({y, ...item}, j) => ({
        y0: y,
        y1:
          mode === modeType.STACK && j !== 0
            ? this.#curveData[i][j - 1].y
            : height + top,
        ...item
      }))
    )
