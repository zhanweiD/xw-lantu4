...
export default class ArcLayer extends LayerBase {
  ...
  setData(tableList, scales = {}) {
    ...
    if (type === waveType.NIGHTINGALEROSE) {
      const percentages = this.#data.select(headers[1])
      percentages.data[0].list = percentages.data[0].list.map(
        () => 1 / percentages.data[0].list.length
      )
      this.#scale = this.createScale(
        {
  ...
  #getLabelData = ({
    ...
    if (labelPosition === labelPositionType.INNER) {
      const [angle, r] = [
        ((startAngle + endAngle) / 360) * Math.PI,
    ...
    if (labelPosition === labelPositionType.OUTER) {
      ...
      const position =
        Math.abs(angle % (2 * Math.PI)) < Math.PI ? "right" : "left"
      return this.createText({
        x: relativeX,
        y: relativeY,
        value,
  ...
  setStyle(style) {
    ...
    const headers = this.#data.data.map(({header}) => header)
    const tableList = this.#data.transpose(
            ...
            (prev, cur, index) => {
              return [
                ...prev,
                {
                  ...
                  outerRadius:
                    prev[index].outerRadius + cur.outerRadius - innerRadius
                }
              ]
            },
    ...
    if (this.#arcData[0]?.length > 1) {
      ...
      this.#arcData.forEach((groupData) =>
