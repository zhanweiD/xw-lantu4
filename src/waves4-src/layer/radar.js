...
export default class RadarLayer extends LayerBase {
  ...
  setData(tableList, scales = {}) {
    ...
    this.#polygonData = pureTableList.map(([dimension, ...values]) => {
      return values.map((value, i) => {
        const [angle, r] = [
          (scaleAngle(dimension) / 180) * Math.PI,
          scaleRadius(value)
        ]
        const [x, y] = [
          polygonCenter.x + Math.sin(angle) * r,
          polygonCenter.y - Math.cos(angle) * r
        ]
        return {
          value,
          dimension,
      ...
      this.#polygonData = this.#polygonData.map((groupData) => {
        return groupData
          .reduce(
            ...
            [{x: polygonCenter.x, y: polygonCenter.y}]
          )
          .slice(1)
      })
    }
  ...
  setStyle(style) {
    ...
    // 颜色跟随主题
    const fillColors = this.getColor(
      this.#polygonData[0].length,
      this.#style.polygon?.fill,
      true
    )
    const strokeColors = this.getColor(
      this.#polygonData[0].length,
      this.#style.polygon?.stroke,
      false
    )
    this.#polygonData.forEach((groupData) =>
  ...
  draw() {
    const polygonData = this.#polygonData[0]
      .map(({fillColor, strokeColor, center}, index) => {
        ...
        const data = this.#polygonData.map((item) => [
          item[index].x,
          item[index].y
        ])
        return {
