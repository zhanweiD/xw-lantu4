...
export default class LayerBase {
  ...
  constructor(layerOptions, waveOptions, subLayers) {
    ...
    this.event = createEvent("@waves4-src/layer")
  ...
  getColor(count, customColors, isMainColor = false) {
    ...
    if (order && this.data instanceof TableList) {
      ...
      const colors = getColor(
        ...
        customColors || this.mainColor
      )
      Object.keys(order).forEach(
        (key) => (colorMapping[key] = colors[order[key]])
      )
      const finalColors = data.slice(1).map(({header}) => colorMapping[header])
      return data.length === 2
        ? new Array(data[0].list.length).fill(finalColors)
        : finalColors
    }
  ...
  createScale(defaultScale, currentScale, incomingStyle) {
    ...
    scaleTypes.forEach((type) => {
      ...
      scale[type] =
        incomingStyle[type] || defaultScale[type] || currentScale[type]
  ...
  createText({
    ...
    value,
    style,
    position = positionType.RIGHTTOP,
    offset = 0,
    textAnchor = "start"
  }) {
    ...
    commonEvents.forEach((eventType) => {
      ...
      this.subLayers.forEach((subLayer) => {
        events[subLayer] = (event, data) =>
          this.event.fire(`${eventType}-${subLayer}`, {event, data})
      })
    })
  }
  ...
  setEvent = (subLayer) => {
    setTimeout(() => {
      const els = this.root
        .selectAll(`.wave-basic-${subLayer}`)
        .style("cursor", "pointer")
      commonEvents.forEach((eventType) =>
        els.on(
          `${eventType}.common`,
          this.#backupEvent.common[eventType][subLayer]
        )
      )
    }, 0)
  }
  ...
  setTooltip(options) {
    ...
    // 为元素绑定事件
    this.tooltip &&
      this.tooltip.options.targets.forEach((subLayer) =>
        setTimeout(() => {
          const els = this.root.selectAll(`.wave-basic-${subLayer}`)
          tooltipEvents.forEach((eventType) =>
            els.on(`${eventType}.tooltip`, this.#backupEvent.tooltip[eventType])
          )
        }, 0)
      )
  }
  ...
  setVisible(isVisiable, subLayer) {
    const targets = subLayer
      ? this.root.selectAll(`.${this.className}-${subLayer}`)
      : this.root
  ...
  setAnimation(options) {
    ...
    this.subLayers.forEach((name) => {
      ...
      loopAnimation &&
        loopQueue.push(
          loopAnimation.type,
          {...loopAnimation, targets},
          this.root
        )
      this.animation[name] = animationQueue
        .push("queue", enterQueue)
        .push("queue", loopQueue)
      // 动画事件注册
      this.animation[name].event.on("start", (data) =>
        this.event.fire(`${name}-animation-start`, data)
      )
      this.animation[name].event.on("process", (data) =>
        this.event.fire(`${name}-animation-process`, data)
      )
      this.animation[name].event.on("end", (data) =>
        this.event.fire(`${name}-animation-end`, data)
      )
    })
    return () =>
      this.subLayers.forEach(
        (type) => this.animation[type] && this.animation[type].play()
      )
  }
  ...
  drawBasic(type, data, subLayer = type) {
    ...
    if (container.size() === 0) {
      container = this.root
        .append("g")
        .attr("class", `${this.className}-${subLayer}`)
    }
    // 分组容器准备，删除上一次渲染多余的组
    for (
      let i = 0;
      i < Math.max(this.#backupData[subLayer].length, data.length);
      i++
    ) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!isEqual(this.#backupData[subLayer][i], data[i])) {
        ...
        const options = {
          ...data[i],
          className: `wave-basic-${subLayer}`,
          container: elContainer
        }
        // 首次渲染不启用数据更新动画
        options.enableUpdateAnimation = this.#backupData[subLayer][i]
          ? data[i].enableUpdateAnimation
          : false
        // 调用基础元素绘制函数进行绘制
