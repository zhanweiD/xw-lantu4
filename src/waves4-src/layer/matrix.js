...
export default class MatrixLayer extends LayerBase {
  ...
  setStyle(style) {
    ...
    // 标签文字数据
    this.#textData = this.#data.get("textData").map((groupData) =>
      groupData.map((item) =>
        this.createText({
          ...item,
          style: text,
    ...
    if (shape === shapeType.CIRCLE) {
      ...
      const [bandWidthX, bandWidthY] = [
        this.#scale.scaleX.bandwidth(),
        this.#scale.scaleY.bandwidth()
      ]
