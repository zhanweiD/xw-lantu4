...
export default class ChordLayer extends LayerBase {
  ...
  setData(table) {
    ...
    const chordData = d3.chord().padAngle(Math.PI / 10 / categorys.length)(
      matrix
    )
    this.#data.set("chordData", chordData)
    // 弧度制转换成角度值
    this.#arcData = chordData.groups.map(
      ({startAngle, endAngle, ...other}, i) => ({
  ...
  setStyle(style) {
    ...
    // 补充边数据
    const ribbonColors = this.getColor(
      this.#arcData.length,
      this.#style.ribbon?.fill,
      true
    )
    this.#ribbonData = this.#data.get("chordData").map(({source, target}) => ({
      ...
      color:
        ribbonColors[
          this.#arcData.findIndex(({index}) => index === target.index)
        ]
    }))
    // 给边数据进行分类
    ...
    this.#ribbonData = this.#ribbonData
      ...
      .slice(1)
    // 标签数据
    this.#textData = this.#arcData.map(
      ({startAngle, endAngle, outerRadius, category}) => {
