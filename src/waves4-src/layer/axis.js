...
const getPosition = (type, targetScale) => {
  ...
  if (targetScale?.type === "band" || targetScale?.type === "point") {
    ...
    targetScale.domain().forEach((label) => {
      position.push([
        label,
        targetScale(label) + (needOffset ? targetScale.bandwidth() / 2 : 0)
      ])
    })
  } else if (targetScale?.type === "linear") {
    const [min, max] = targetScale.domain()
    d3.range(min, max + 1, (max - min) / targetScale.nice.count).forEach(
      (label) => {
...
export default class AxisLayer extends LayerBase {
  ...
  setData(data, scale) {
    ...
    if (type === axisType.ANGLE || type === axisType.POLAR) {
      ...
      const maxRadius =
        this.#scale.scaleRadius?.range()[1] || Math.max(width / 2, height / 2)
  ...
  setStyle(style) {
    ...
    const {labelOffset, textX, textXT, textY, textYR, textAngle, textRadius} =
      this.#style
    // X轴主轴标签在线的正下方
    ...
    this.#textData.textY = this.#lineData.lineY.map(({value, x1, y1}) => {
      const position =
        this.#scale.scaleY?.type === "linear" ? "right-bottom" : "right"
      return this.createText({
        x: x1,
        y: y1,
        position,
    ...
    // 角度坐标在线的延伸处
    this.#textData.textAngle = this.#lineData.lineAngle.map(
      ({value, x2, y2, angle}) => {
    ...
    // 半径坐标在角度坐标第一根线的右侧
    this.#textData.textRadius = this.#lineData.lineRadius.map(
      ({value, cx, cy, rx}) => {
        return this.createText({
          x: cx,
          y: cy - rx,
  ...
  draw() {
    ...
    const transformLineData = (key) => [
      {
        position: this.#lineData[key].map(({x1, y1, x2, y2}) => [
          x1,
          y1,
          x2,
          y2
    ...
    scaleX?.type === "linear" &&
      this.drawBasic("line", transformLineData("lineX"), "lineX")
    scaleXT?.type === "linear" &&
      this.drawBasic("line", transformLineData("lineXT"), "lineXT")
    scaleY?.type === "linear" &&
      this.drawBasic("line", transformLineData("lineY"), "lineY")
    scaleYR?.type === "linear" &&
      this.drawBasic("line", transformLineData("lineYR"), "lineYR")
