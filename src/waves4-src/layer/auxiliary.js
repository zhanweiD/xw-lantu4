...
export default class AuxiliaryLayer extends LayerBase {
  ...
  setData(data, scale) {
    ...
    this.#lineData = this.#data.map((value) => ({
      ...
      x1:
        left +
        (type === directionType.HORIZONTAL ? 0 : this.#scale.scaleX(value)),
      y1:
        top +
        (type === directionType.HORIZONTAL ? this.#scale.scaleY(value) : 0),
      x2:
        left +
        (type === directionType.HORIZONTAL ? width : this.#scale.scaleX(value)),
      y2:
        top +
        (type === directionType.HORIZONTAL ? this.#scale.scaleY(value) : height)
    }))
  }
