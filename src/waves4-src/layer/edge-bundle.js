...
export default class EdgeBundleLayer extends LayerBase {
  ...
  setData(relation) {
    ...
    this.#curveData = this.#data.data.links.map(({from, to}) => {
      ...
      const [angle1, angle2] = [
        (source.x / 180) * Math.PI,
        (target.x / 180) * Math.PI
      ]
      const [radius1, radius2] = [source.y, target.y]
      const [x1, y1] = [
        Math.sin(angle1) * radius1 + centerX,
        centerY - Math.cos(angle1) * radius1
      ]
      const [x2, y2] = [
        Math.sin(angle2) * radius2 + centerX,
        centerY - Math.cos(angle2) * radius2
      ]
      const [curveX, curveY] = [
        (x1 + x2) / 4 + centerX / 2,
        (y1 + y2) / 4 + centerY / 2
  ...
  setStyle(style) {
    ...
    const circleColors = this.getColor(
      categorys.length,
      this.#style.cirle?.fill,
      true
    )
    this.#circleData.forEach((groupData) =>
      groupData.forEach((item) => {
        item.color =
          circleColors[
            categorys.findIndex((value) => value === item.source.category)
          ]
      })
    )
    const curveColors = this.getColor(
      categorys.length,
      this.#style.curve?.fill,
      false
    )
    this.#curveData.forEach((item) => {
      item.color =
        curveColors[categorys.findIndex((value) => value === item.category)]
    })
    // 标签数据
    this.#textData = this.#circleData.map((groupData) =>
      groupData.map(({r, source, angle, radius}) => {
        const totalRadius = r + radius + labelOffset
        const textAnchor = angle > Math.PI ? "end" : "start"
        const [x, y] = [
          Math.sin(angle) * totalRadius + centerX,
          centerY - Math.cos(angle) * totalRadius
        ]
        const data = this.createText({
          x,
          y,
          value: source.name,
          position: "right",
        ...
        return {
          ...data,
          textAnchor,
          angle: (angle / Math.PI) * 180 - (angle > Math.PI ? 270 : 90)
        }
      })
    )
  }
  ...
  draw() {
    const circleData = this.#circleData.map((groupData) => {
      ...
      const source = groupData.map((item) => ({
        ...item.source,
        dimension: item.source.name
      }))
      const fill = groupData.map(({color}) => color)
      return {data, position, source, ...this.#style.circle, fill}
    })
    const curveData = this.#curveData.map(
      ({x1, y1, x2, y2, curveX, curveY, color}) => ({
        position: [
          [
            [x1, y1],
            [curveX, curveY],
