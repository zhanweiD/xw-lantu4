...
export default class SankeyLayer extends LayerBase {
  ...
  setStyle(style) {
    ...
    const maxNumbers = levels.map((level) => {
      ...
      const ratio =
        totalNumber /
        ((isHorizontal ? layout.height : layout.width) - gapLength)
    ...
    this.#rectData = this.#rectData.map((groupData) => {
      return groupData
        .reduce(
          ...
          [{y: groupData[0].y - nodeGap, height: 0}]
        )
        .slice(1)
    })
    // 对齐调整节点的位置
    ...
    if (type === directionType.VERTICAL) {
      this.#rectData = this.#rectData.map((groupData) =>
        groupData.map(({x, y, height, width, ...other}) => ({
          width: height,
          height: width,
      ...
      this.#ribbonData = this.#ribbonData.map(
        ({x1, y1, x2, y2, x3, y3, x4, y4, ...other}) => ({
          ...other,
    ...
    this.#textData = this.#rectData.map((groupData, i) => {
      ...
      if (type === directionType.HORIZONTAL) {
        return groupData.map(({x, y, width, height, name, value}) =>
          this.createText({
  ...
  draw() {
    ...
    const ribbonData = this.#ribbonData.map(
