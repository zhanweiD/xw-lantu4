...
const getRealData = (dataSource, mappingValues) => {
  ...
  const dataSet = keys(mappingValues).map((groupId) => {
    ...
    return fields.map(({value}) =>
      value.map(({key}) => {
        const targetIndex = dataSource[sourceId][0].findIndex(
          (header) => header === key
        )
  ...
  const tableLists = dataSet.map((item) =>
    dataBase.transpose(item.reduce((prev, cur) => [...prev, ...cur]))
  )
...
function translate(schema) {
  ...
  // 处理图层配置
  const layerConfig = layers.map(
    ({type, id, data, axis, children, other, tooltip}) => {
      ...
      if (tooltip && tooltip.useTooltip) {
        merge(_tooltip, {
          ...tooltip,
          targets: children.map(({option}) => option)
        })
      }
      return {
        type,
