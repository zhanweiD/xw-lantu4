...
const isDependentLayer = (layerType) =>
  dependentLayers.find((type) => type === layerType)
const isNormalLayer = (layerType) =>
  !dependentLayers.find((type) => type === layerType) && layerType !== "axis"
...
const createLayer = (wave, config) => {
  ...
  const layer = wave.createLayer(type, {
    ...options,
    layout: wave.layout[options.layout]
  })
  // 数据结构判断
  let dataSet = data
  if (
    isArray(data) &&
    data.length === 2 &&
    DataBase.isRelation(data[0], data[1])
  ) {
    ...
    dataSet = new TableList(
      DataBase.isTableList(data) ? data : Random.tableList(data)
    )
    // 列表到表格的转换
    if (type === "matrix" || type === "chord") {
      dataSet = new Table(
        DataBase.tableListToTable(
          DataBase.isTableList(data) ? data : Random.tableList(data)
        )
      )
    }
  ...
  if (type === "legend") {
    dataSet = wave.layer
      .filter(({instance}) => instance.data instanceof TableList)
...
function createWave(schema, existedWave) {
  try {
    ...
    const dependentLayerConfigs = layers.filter(({type}) =>
      isDependentLayer(type)
    )
    const normalLayers = normalLayerConfigs.map((layer) =>
      createLayer(wave, layer)
    )
    ...
    layers.map(({options}) =>
      wave.layer.find(({id}) => id === options.id).instance.draw()
    )
