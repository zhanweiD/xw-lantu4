...
export default function Scale({type, domain, range, nice = defaultNice}) {
  ...
  if (type === "band") {
    ...
    if (isNumber(nice?.fixedBandWidth) && isNumber(nice?.fixedPaddingInner)) {
      ...
      const totalRange =
        fixedBandWidth * domain.length + fixedPaddingInner * (domain.length - 1)
      const offset =
        (totalRange - Math.abs(range[1] - range[0])) *
        (range[1] > range[0] ? 1 : -1)
      const [fixedStart, fixedEnd] = [
        fixedBoundary === "start",
        fixedBoundary === "end"
      ]
      // 无自适应，调整修改值域
      ...
      scale.paddingInner(
        fixedPaddingInner / (fixedPaddingInner + fixedBandWidth)
      )
    } else if (isNumber(nice?.fixedBandWidth)) {
      // 只固定带宽，间距自适应
      scale.paddingInner(
        1 -
          (nice.fixedBandWidth * domain.length) / Math.abs(range[1] - range[0])
      )
    } else if (isNumber(nice?.fixedPaddingInner)) {
      // 只固定间距，带宽自适应
      scale.paddingInner(
        (nice.fixedPaddingInner * (domain.length - 1)) /
          Math.abs(range[1] - range[0])
      )
    } else if (nice) {
      // 间距和带宽都根据比例自适应
      scale.paddingInner(
        isNumber(nice.paddingInner)
          ? nice.paddingInner
          : defaultNice.paddingInner
      )
    }
  }
  ...
  if (type === "angle") {
    ...
    const padding =
      (totalLength * nice.paddingInner) / domain.data[0].list.length
  ...
  if (tickCount > 0) {
    ...
    if (newEnd > end) {
      ...
      const isExceedThreshold = () =>
        (newEnd - end) / (newEnd - newStart) > spaceThreshold
