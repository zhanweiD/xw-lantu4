...
const tableList = (options) => {
  ...
  const numbers = new Array(row * column)
    .fill()
    .map(() => toFixed(getNumber(), decimalPlace))
  const headers = ["维度"].concat(
    new Array(column).fill().map((v, i) => `第${i + 1}类`)
  )
  const lists = new Array(row)
    .fill()
    .map((v, i) =>
      [`第${i + 1}项`].concat(numbers.slice(i * column, (i + 1) * column))
    )
...
const table = (options) => {
  ...
  const numbers = new Array(row)
    .fill()
    .map(() =>
      new Array(column).fill().map(() => toFixed(getNumber(), decimalPlace))
    )
