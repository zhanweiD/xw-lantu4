...
export default class Table extends DataBase {
  ...
  select(rows, columns) {
    ...
    const rowsIndex = _rows.map((row) =>
      this.data[0].findIndex((value) => value === row)
    )
  ...
  remove(target = targetType.ROW, ...data) {
    ...
    data.forEach((dimension) => {
      if (target === targetType.ROW) {
        ...
        index !== -1 &&
          removedList.concat(
            this.data[2].map((item) => item.splice(index, 1)[0])
          )
      }
    })
    return removedList
  ...
  sort() {
    ...
    // 返回二维数组形式的排序顺序
    order.forEach(
      (value, i) => (result[Math.floor(value / column)][value % column] = i)
    )
    return result
  }
}
