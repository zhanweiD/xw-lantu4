...
export default class Relation extends DataBase {
  ...
  update(nodeTableList, linkTableList) {
    if (!this.isLegalData("relation", nodeTableList, linkTableList)) {
      ...
      const nodeNameIndex = nodeTableList[0].findIndex(
        (value) => value === "name"
      )
      const nodeValueIndex = nodeTableList[0].findIndex(
        (value) => value === "value"
      )
      const nodeCategoryIndex = nodeTableList[0].findIndex(
      ...
      // 边数据
      const linkFromIndex = linkTableList[0].findIndex(
        (value) => value === "from"
      )
      const linkToIndex = linkTableList[0].findIndex((value) => value === "to")
      const linkValueIndex = linkTableList[0].findIndex(
        (value) => value === "value"
      )
      ...
      if (nodeValueIndex === -1 && linkValueIndex !== -1) {
        this.data.nodes.forEach((node) => {
          const froms = this.data.links
            .filter(({from}) => from === node.id)
            .map(({value}) => value)
          const tos = this.data.links
