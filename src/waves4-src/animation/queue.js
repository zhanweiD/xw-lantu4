...
export default class AnimationQueue extends AnimationBase {
  ...
  connect(priorityConfig) {
    ...
    // 根据优先级分组动画
    const groupedAnimationQueue = new Array(Math.max(...finalPriority) + 1)
      .fill()
      .map(() => [])
    ...
    // 对分组后的动画进行连接
    groupedAnimationQueue.reduce(
      (previousAnimations, currentAnimations, priority) => {
        // 子动画的每个生命周期都是动画序列的 “process”
        currentAnimations.forEach((animation) => {
          const mapToState = (state) => ({id: animation.id, priority, state})
          const [startState, processState, endState] = [
            "start",
            "process",
            "end"
          ].map(mapToState)
          animation.instance.event.on("start", () => this.process(startState))
          animation.instance.event.on("process", (data) =>
        ...
        if (priority === Math.max(...finalPriority))
          this.bind(currentAnimations, () => this.end())
        // 前一优先级动画全部结束后启动当前优先级的动画
        this.bind(previousAnimations, () =>
  ...
  push(type, options, context) {
    ...
    if (type === "function") {
      ...
      createQueueableAnimation(
        new AnimationMap[type]({...options, loop: false}, context)
      )
    } else {
  ...
  play() {
    if (this.isAnimationStart || !this.isAnimationAvailable) {
      this.isAnimationStart &&
        this.log.error("The animation is already started!")
      !this.isAnimationAvailable &&
        this.log.error("The animation is not available!")
    } else {
      // 重新连接，开始动画
