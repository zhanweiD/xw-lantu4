...
export default class ScanAnimation extends AnimationBase {
  ...
  play() {
    const {delay, duration, direction, scope, loop, className, context} =
      this.options
    const attributes = getAttributes(direction)
    const isLeftOrTop =
      direction === directions.LEFT || direction === directions.TOP
    ...
    if (this.isAnimationFirstPlay) {
      ...
      this.lights = context
        ...
        .attr(
          "stroke",
          scope !== scopes.FILL ? `url(#scanAnimation${count})` : ""
        )
        .style(
          "fill",
          scope !== scopes.STROKE ? `url(#scanAnimation${count})` : ""
        )
      ...
      const observers = targetNode.forEach((el, i) => {
        return new MutationObserver((mutationList) =>
          mutationList.forEach(({attributeName}) => {
            d3.select(this.lights._groups[0][i]).attr(
              attributeName,
              d3.select(el).attr(attributeName)
            )
          })
