...
import errorMap from "./exhibit-error"
...
const createExhibitAdapter = (hooks) =>
  class Adapter {
    ...
    warn({type, error, data}) {
      const errorInfo = errorMap[type]
      let text
      let onTextClick
      if (errorInfo) {
        text = errorMap[type].text
        onTextClick = () => errorMap[type].onClick({error, data})
      } else {
        text = type
        onTextClick = () => {
          console.log(data)
      ...
      if (isFunction(hooks.warn)) {
        hooks.warn.call(this, {
          instance: this.instance,
      ...
      if (this.ready === false) {
        this.event.fire("ready", "adapter warn")
      }
    }
  }
